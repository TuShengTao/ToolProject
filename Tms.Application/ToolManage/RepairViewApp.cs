...
namespace Tms.Application.ToolManage
{
    public class RepairViewApp
    {
        ...
        public List<RepairViewEntity> GetList(Pagination pagination, string keyword,string searchType)
        {
            ...
            if (searchType == "已通过")
            {
                expression = expression.And(t => t.T_Stauts.Equals(1));
            }
            if (searchType == "未通过")
            {
                expression = expression.And(t => t.T_Stauts.Equals(-1));
            ...
            if (searchType == "修复已完成")
            {
                expression = expression.And(t => t.T_RepairedStatus.Equals(1));
            }
            if (searchType == "修复未完成")
            {
                expression = expression.And(t => t.T_RepairedStatus.Equals(0));
            }
